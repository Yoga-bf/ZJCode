...
int create_table_top_company(char* table_name);
...
int main()
{
	...
	if ( NULL == mysql_real_connect(&mysql, "localhost", "root", "hengheng", "OID", 0, NULL, 0)){
...
int create_table_top()
{
    ...
    for(i = 0; i < 190; i++)
    {
        ...
        i7 = random_1(1,9);
	i8 = random_1(1,9);
        sprintf(key, "%c%c%c%c%c%c%c%c", name_space[i1],name_space[i2],name_space[i3],name_space[i4],name_space[i5],name_space[i6],name_space[i7],name_space[i8]);
        ...
        k = create_table_top_company(match);
        if( k == -1 ){
            free(key);
            free(match);
            return -1;
